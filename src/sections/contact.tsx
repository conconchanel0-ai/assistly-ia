...
const PROMISES = [
  "Devis gratuit, adapté à vos besoins et sans engagement.",
  "50 % de réduction sur de nombreuses prestations, au titre des services à la personne.",
  "Accompagnement gratuit dans vos démarches d'inscription.",
  "Interventions jusqu'à 25 km autour de Bains-les-Bains, particuliers et entreprises.",
];
...
export function Contact() {
  ...
  return (
    <Slide id="devis" tone="blue" n={9} headerRight="Devis gratuit">
      <div className="mt-6 grid gap-8 sm:mt-8 lg:grid-cols-[1fr_1fr] lg:gap-14">
        <div>
          ...
          <Reveal as="p" delay={90} className="type-editorial mt-5 max-w-[58ch] text-[11.5px] text-cream/90 sm:text-[12.5px]">
            Dites-nous où vous êtes et ce dont votre jardin a besoin : une taille,
            une remise en état, un massif, un potager, ou un entretien suivi. On
            vous rappelle pour convenir d'un rendez-vous, puis le devis part par
            mail.
          </Reveal>
        ...
        {state === "sent" ? (
          <div className="flex flex-col justify-center border border-cream/35 p-6 sm:p-8">
            ...
            <p className="type-editorial mt-3 text-[11.5px] text-cream/90 sm:text-[12.5px]">
              On revient vers vous très rapidement. Pour une demande urgente, le
              téléphone reste le plus sûr : {BRAND.phone}.
            </p>
          </div>
        ) : (
          <form onSubmit={onSubmit} noValidate className="flex flex-col gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field id="f-commune" name="commune" label="Votre commune" required autoComplete="address-level2" placeholder="Bains-les-Bains" />
              <Field id="f-objet" name="prestation" label="Votre demande" required placeholder="Taille de haie, tonte, potager…" />
            </div>
            <p className="flex flex-col gap-1.5">
              <label
                htmlFor="f-msg"
                className="font-body text-[10px] font-bold uppercase tracking-[0.16em] text-cream/70"
              >
                Quelques mots sur votre jardin
              </label>
              <textarea
                id="f-msg"
                name="message"
                rows={4}
                placeholder="Surface, état actuel, longueur de haie, délai souhaité, questions…"
            ...
            >
              {state === "error"
                ? `L'envoi a échoué. Réessayez, ou appelez le ${BRAND.phone}.`
                : `Réponse rapide · ${BRAND.phone} · ${BRAND.site}`}
            </p>
          </form>
        )}
      </div>
      <Mention tone="cream">
        Formulaire de démonstration : les demandes ne parviennent pas encore à
        l'entreprise. Le téléphone {BRAND.phone}, l'adresse {BRAND.email} et le
        site {BRAND.site} sont, eux, ceux publiés par Dis Moi Nature.
      </Mention>
    </Slide>
  );
}
