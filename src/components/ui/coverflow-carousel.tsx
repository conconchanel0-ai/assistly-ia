...
export interface CoverflowCarouselProps {
  ...
  /** Card index the carousel opens on. */
  initialIndex?: number;
  /** Called whenever the centred card changes. */
...
export function CoverflowCarousel({
  ...
  /** Un glissement ne doit pas être compris comme un clic sur une carte. */
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Au-delà de quelques pixels, c'est un glissement, plus un clic.
  ...
  /**
   * Clic à droite de la carte centrale : on avance d'une image ; à gauche : on
   * recule. Traité au niveau du cadre, donc la zone vide autour des vignettes
   * répond aussi, ce qui compte surtout sur téléphone.
   */
  const onFrameClick = (event: React.MouseEvent<HTMLDivElement>) => {
    if (movedRef.current) return;
    const rect = event.currentTarget.getBoundingClientRect();
    const dx = event.clientX - (rect.left + rect.width / 2);
    const half = (widthRef.current || rect.width * 0.4) / 2;
    // La carte centrale ne déclenche rien : elle est déjà à sa place.
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                // Le déplacement est décidé par le cadre (onFrameClick), qui
                // sait de quel côté du centre le clic est tombé.
                className={cn(
